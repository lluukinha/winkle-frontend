...
<template>
  ...
  >
    <div class="header-top flex justify-between items-center">
      ...
      >
        <span class="truncate">{{ LoginRepository.loginData.value.user }}</span>
      ...
      <div
        ...
        class="w-screen h-screen absolute bg-black opacity-20 inset-0 z-0"></div>
      <div
        class="
          fixed z-50 p-2
          top-10 right-10
      ...
      >
        <div class="py-3 px-4">
          ...
          >{{ LoginRepository.loginData.value?.user }}</span>
          <span
            class="block text-sm font-medium text-gray-500 truncate dark:text-gray-400"
          >{{ LoginRepository.loginData.value?.login }}</span>
        </div>
        <div class="flex justify-center items-center p-2 text-sm text-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white cursor-pointer">
          <DarkModeSelector />
        </div>
        <ul class="py-1" aria-labelledby="dropdown">
        <li @click="() => { isMenuOpen = false; $router.push({ name: 'dashboard-user' }); }">
