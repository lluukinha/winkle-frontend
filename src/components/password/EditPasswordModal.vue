...
defineProps<{ password: IPassword }>();
...
const handleSave = (updatedPassword: IPassword) => {
  ...
  if (folderName !== '') {
    ...
    const currentFolder = PasswordStore.foldersList.value.find(
      (f: IFolder) => f.name.toUpperCase() === folderName.toUpperCase()
    );
