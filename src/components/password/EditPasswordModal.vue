...
const foldersList = computed(() => {
  const folderName = updatedPassword.value.folder.name;
...
const selectFolder = (folder: IFolder) => {
  updatedPassword.value.folder = folder;
};
...
onBeforeMount(() => {
  updatedPassword.value.folder = { id: '', name: '' };
});
...
const handleSave = (e: Event) => {
  ...
  if (willChange) updatedPassword.value.url = `https://${url}`;

  WinkleScripts.setLoading(true);
  PasswordRepository.updatePassword(updatedPassword.value)
    .then((newPassword: IPassword) => {
      const willReloadFolders = updatedPassword.value.folder.id !== updatedPassword.value.folderId;
...
<template>
  <Modal @close="handleClose()" @save="sendForm()">
    ...
    <form class="w-full" autocomplete="off" @submit="handleSave">
      ...
      <div class="md:flex md:items-center mb-6">
        ...
        <div class="md:w-2/3">
          <input
            ...
            type="text"
            v-model="updatedPassword.folder.name"
      ...
      <div
        ref="foldersDropdown"
        class="w-48 border bg-gray-50 absolute rounded shadow-lg -mt-6 md:ml-40"
