...
const doLogin = (e: Event) => {
  ...
  LoginRepository.doLogin(loginForm.value)
    .then(async () => {
      await LoginRepository.setMasterPassword(loginForm.value.password);
      emit('loginFinished');
    })
    .catch((e: AxiosError) => {
      const unauthorizedError = 'Unauthorized';
      const isUnauthorized =
        e.response?.statusText === unauthorizedError ||
...
<template>
  <form @submit="doLogin">
    <div class="text-left">
      ...
      <input
        ref="firstInput"
        class="w-full text-lg p-2 border-b border-gray-300 focus:outline-none focus:border-gray-200 bg-gray-100 rounded"
        type="email"
    ...
    <div class="mt-8">
      <div class="flex justify-between items-center">
        ...
        <div>
          <span
            @click="$emit('forgot')"
            class="text-xs font-display font-semibold text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-gray-100 cursor-pointer"
          >
      ...
      <div class="w-full flex">
        <input
          class="w-full text-lg p-2 border-b border-gray-300 bg-gray-100 border-r-0 focus:outline-none focus:border-gray-200 rounded rounded-tr-none rounded-br-none"
    ...
    <div class="mt-10">
      <button
        class="bg-gray-600 hover:bg-gray-700 text-gray-100 p-4 w-full rounded-full tracking-wide font-semibold font-display focus:outline-none focus:shadow-outline shadow-lg"
      >
        {{ $t('login.enter') }}
      </button>
    </div>
